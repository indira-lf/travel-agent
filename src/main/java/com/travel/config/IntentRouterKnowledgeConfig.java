...
import com.travel.agent.intent.IntentVectorMatcher;
...
public class IntentRouterKnowledgeConfig {
    ...
    public EmbeddingModel intentRouterEmbeddingModel() {
        if (apiKey == null || apiKey.isBlank()) {
            logger.warn("[INTENT_ROUTER] 未配置 agentscope.dashscope.api-key，跳过意图路由向量库装配，L2 将禁用（降级到 L1+L3）");
            return null;
        }
    ...
    public Knowledge intentRouterKnowledge(ObjectProvider<EmbeddingModel> embeddingModelProvider) {
        EmbeddingModel intentRouterEmbeddingModel = embeddingModelProvider.getIfAvailable();
        if (intentRouterEmbeddingModel == null) {
            return null;
        }
        ...
        try {
            ...
            // DashScope Key 失效或网络异常时优雅降级：L2 禁用，由 L1 规则 + L3 LLM 兜底
            logger.warn("[INTENT_ROUTER] 意图路由向量库预填充失败，L2 将禁用（降级到 L1+L3）。原因：{}", e.getMessage());
            return null;
        }
     ...
     * 这里通过 {@link Qualifier} 精确指定 {@code intentRouterKnowledge} 依赖。</p>
     *
     * <p>使用 {@link ObjectProvider} 以便在 {@code intentRouterKnowledge} 未装配
     * （DashScope Key 未配置/失效，bean 返回 null 被过滤）时优雅拿到 {@code null}，
     * 避免 {@code NoSuchBeanDefinitionException} 阻塞启动。</p>
     */
    @Bean(name = "intentVectorMatcher")
    public IntentVectorMatcher intentVectorMatcher(
            @Qualifier("intentRouterKnowledge") ObjectProvider<Knowledge> knowledgeProvider) {
        Knowledge intentRouterKnowledge = knowledgeProvider.getIfAvailable();
        if (intentRouterKnowledge == null) {
            logger.warn("[INTENT_ROUTER] intentRouterKnowledge 未装配，IntentVectorMatcher 将禁用，L2 跳过");
            return null;
        }
