...
public class WeatherMcpConfig {
    ...
    public McpClientWrapper weatherMcpClient() {
        ...
        // 未配置端点时不装配天气 MCP，超过 3 天的天气查询将不可用（优雅降级）
        if (weatherMcpEndpoint == null || weatherMcpEndpoint.isBlank()) {
            logger.warn("[WeatherMcp] 未配置 app.weather-mcp.endpoint，跳过天气 MCP 客户端初始化");
            return null;
        }
